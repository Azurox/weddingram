...
const heartAnimationTimeout = ref<NodeJS.Timeout | null>(null)

function onImageClick() {
  const now = new Date().getTime()
  const timesince = now - latestClickTime
  if ((timesince < 200) && (timesince > 0)) {
    toggleFavorite(picture.id)

    if (heartAnimationTimeout.value) {
      clearTimeout(heartAnimationTimeout.value)
      heartAnimationTimeout.value = null
    }

    if (isInFavorite(picture.id)) {
...
function triggerHeartAnimation() {
  ...
  heartAnimationTimeout.value = setTimeout(() => {
...
.animate-heart-like {
  animation: heart-like 0.8s ease-out forwards;
}
</style>
