...
async function handleMultiRemoveFavorite() {
  ...
  <div class="relative">
    ...
    <template v-else>
      <UiContainer class="grid grid-cols-3 gap">
        <div v-for="picture in favoritePictureList" :key="picture.id" class="relative duration-200 transition-all" :class="{ 'ring-4 ring-merino-400 ring-opacity-70 z-10': selectedPictures.has(picture.id) }">
    ...
    <Transition name="slide-in">
      ...
      >
        <div class="relative">
          <button
            type="button"
            class="group bg-almond-500 hover:bg-almond-600 text-white p-4 rounded-full shadow-lg transition-all duration-200 transform pointer-events-auto flex items-center justify-center active:scale-95 disabled:pointer-events-none disabled:opacity-60"
        ...
        <div class="relative">
          ...
          >
            <svg class="size-6 text-white" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><!-- Icon from Material Symbols by Google - https://github.com/google/material-design-icons/blob/master/LICENSE --><path fill="currentColor" d="m12 16l-5-5l1.4-1.45l2.6 2.6V4h2v8.15l2.6-2.6L17 11zm-8 4v-5h2v3h12v-3h2v5z" /></svg>
          </button>
        </div>
      </UiContainer>
    </Transition>
